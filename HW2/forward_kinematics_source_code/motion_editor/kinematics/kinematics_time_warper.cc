        ...
        )
{
    *hard_constraint_coll_ = hard_constraint_coll;
    ...
    int32_t warped_desired_catch_ball_frame_idx = frame_per_second * hard_constraint_coll_->at(1).play_second; //150
    int32_t desired_catch_ball_frame_idx = hard_constraint_coll_->at(1).frame_idx; //160

    float frame_step_before_catch_ball = (float)warped_desired_catch_ball_frame_idx / (float)desired_catch_ball_frame_idx; //0.97
    float frame_step_after_catch_ball = (float)(hard_constraint_coll_->at(2).frame_idx - warped_desired_catch_ball_frame_idx) /
        (float)(hard_constraint_coll_->at(2).frame_idx - desired_catch_ball_frame_idx); //1.34
